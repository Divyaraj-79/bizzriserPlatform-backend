...
let WhatsappService = WhatsappService_1 = class WhatsappService {
    ...
    async uploadMedia(orgId, accountId, file) {
        ...
        if (file.size > limit) {
            ...
            throw new common_1.HttpException(errorMsg, common_1.HttpStatus.PAYLOAD_TOO_LARGE);
        }
        try {
            const fs = require('fs');
    ...
    async sendInteractiveMessage(orgId, accountId, to, interactivePayload) {
        const account = await this.prisma.whatsAppAccount.findUnique({
            where: { id: accountId, organizationId: orgId },
        });
        if (!account)
        ...
        try {
            ...
            for (const mt of metaTemplates) {
                ...
                if (!existing) {
                    ...
                    const hasChanged = existing.status !== (mt.status || 'APPROVED') ||
