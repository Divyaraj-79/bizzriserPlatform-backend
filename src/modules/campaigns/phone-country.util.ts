/**
 * Phone Number to Country Code Resolver
 *
 * Uses E.164 phone number format (e.g. +91XXXXXXXXXX) to determine the country.
 * Handles ambiguous prefixes (e.g. +1 for US/CA, +7 for RU/KZ).
 * Falls back to DEFAULT_COUNTRY_CODE (IN) when resolution fails.
 */

import { DEFAULT_COUNTRY_CODE } from './whatsapp-pricing.data';

/**
 * Sorted prefix map: longer prefixes first for greedy matching.
 * Format: [prefix_digits, countryCode]
 */
const PHONE_PREFIX_MAP: [string, string][] = [
  // 4-digit prefixes (specific regions)
  ['1242', 'BS'], ['1246', 'BB'], ['1264', 'AI'], ['1268', 'AG'],
  ['1284', 'VG'], ['1340', 'VI'], ['1345', 'KY'], ['1441', 'BM'],
  ['1473', 'GD'], ['1649', 'TC'], ['1664', 'MS'], ['1670', 'MP'],
  ['1671', 'GU'], ['1684', 'AS'], ['1758', 'LC'], ['1767', 'DM'],
  ['1784', 'VC'], ['1787', 'PR'], ['1809', 'DO'], ['1868', 'TT'],
  ...
  // 1-digit prefixes (handled last — most ambiguous)
  ['1', 'US'],  // +1 for US and CA; defaults to US
  ['7', 'RU'],  // +7 for RU and KZ; defaults to RU
];

/**
 * Resolve a phone number string to an ISO 3166-1 alpha-2 country code.
 * @param phone - E.164 format preferred (e.g. +919876543210), but digits-only also accepted.
 */
export function resolveCountryFromPhone(phone: string): string {
  ...
  // If phone doesn't start with country code (no +), try to handle short numbers gracefully
  // by treating them as Indian numbers (default country)
  if (!phone.startsWith('+') && digits.length <= 10) {
    return DEFAULT_COUNTRY_CODE;
  }

  // Greedy match: try longest prefix first (already sorted: 4-digit, 3-digit, 2-digit, 1-digit)
...
/**
 * Resolve countries for an array of phone numbers.
 * Returns a map of { countryCode -> count }.
 */
export function groupPhonesByCountry(phones: string[]): Record<string, number> {
  const result: Record<string, number> = {};
  for (const phone of phones) {
    const code = resolveCountryFromPhone(phone);
